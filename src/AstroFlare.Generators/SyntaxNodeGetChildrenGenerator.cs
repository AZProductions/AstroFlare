...
namespace AstroFlare.Generators
{
    [Generator]
...
#pragma warning disable IDE0063 // Use simple 'using' statement: we want to control when the 'using' varibales go out of scope
            ...
            var separatedSyntaxListType = compilation.GetTypeByMetadataName("AstroFlare.CodeAnalysis.Syntax.SeparatedSyntaxList`1");
            var syntaxNodeType = compilation.GetTypeByMetadataName("AstroFlare.CodeAnalysis.Syntax.SyntaxNode");
            ...
            using (var indentedTextWriter = new IndentedTextWriter(stringWriter, indentString))
            {
                ...
                using (var nameSpaceCurly = new CurlyIndenter(indentedTextWriter, "namespace AstroFlare.CodeAnalysis.Syntax"))
                {
